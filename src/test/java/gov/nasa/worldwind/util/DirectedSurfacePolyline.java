...
public class DirectedSurfacePolyline extends SurfacePolyline {
    ...
    /**
     * Computes the geometry of the direction arrows. Called from {@link #drawOutline(DrawContext,
    ...
    protected void computeArrowheadGeometry(DrawContext dc, SurfaceTileDrawContext sdc, LatLon begin, LatLon end) {
        ...
        if ((this.getPathType() == null || this.getPathType().equals(Keys.LINEAR)) &&
            ...
            double datelineOffset = begin.lon < 0 ? -360 : 360;
            LatLon falseEnd = LatLon.fromDegrees(end.lat, end.lon + datelineOffset);
        ...
        for (List<LatLon> list : drawLocations) {
            ...
            for (LatLon location : list) {
                vertexBuffer.put((float) (location.lon - referenceLocation.lon));
                vertexBuffer.put((float) (location.lat - referenceLocation.lat));
            }
        }
    }

    /**
     * Draws this DirectedSurfacePolyline's direction arrows. Called from {@link #drawOutline(DrawContext,
     * SurfaceTileDrawContext)} after drawing the polyline's actual outline.
     *
     * @param dc the current draw context.
     */
