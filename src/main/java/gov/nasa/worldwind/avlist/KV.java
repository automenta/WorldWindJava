...
/**
 * An interface for managing a key,value pair collection.
 *
 * @author Tom Gaskins
...
public interface KV {
     ...
     *
     * @param key   the key name. May not be <code>null</code>.
     * @param value the key value. May be <code>null</code>, in which case any existing value for the key is
     *              removed from the collection.
     * @return previous value associated with specified key, or null  if there was no mapping for key. A null return can
     * also indicate that the map previously associated null  with the specified key, if the implementation supports
     * null values.
     ...
     *
     * @param key the key name. May not be <code>null</code>.
     * @return the key value if one exists in the collection, otherwise <code>null</code>.
     * @throws NullPointerException if <code>key</code> is <code>null</code>.
     */
    ...
    /**
     * Adds the contents of another key-value list to the list. Replaces an existing key/value pair if the list
    ...
    default boolean isEmpty() {
        return !getValues().iterator().hasNext();
    }
}