...
public class GeographicImageInterpolator extends ImageInterpolator {
    ...
    /**
     * Overridden to create a {@link GeographicImageInterpolator.GeographicCell}.
     *
    ...
    protected static class GeographicCell extends Cell {
        ...
        public boolean intersects(float x, float y) {
            ...
            return ((x >= this.minx && x <= 180.0f) || (x >= -180.0f && x <= this.maxx))
        ...
        protected void computeExtremesFromLocations(Dimension dim, float[] xs, float[] ys) {
            ...
            for (int j = this.n0; j <= this.n1; j++) {
                for (int i = this.m0; i <= this.m1; i++) {
                    ...
                    if (this.minx > x && x > 0.0f)
                        this.minx = x;
                    if (this.maxx < x && x < 0.0f)
                        this.maxx = x;
        ...
        protected void computeExtremesFromChildren() {
            ...
            for (Cell t : this.children) {
                ...
                // cell's minx and maxx must extent to the prime meridian to include it.
                    ...
                    if (this.minx > t.minx && t.minx > 0.0f) // Cell is entirely within the eastern hemisphere.
                        this.minx = t.minx;
                    if (this.maxx < t.maxx && t.maxx < 0.0f) // Cell is entirely within the western hemisphere.
                        this.maxx = t.maxx;
                    if (t.minx <= 0.0f && t.maxx >= 0.0f) // Cell is in both the western and eastern hemispheres.
                        this.minx = this.maxx = 0.0f;
                }
