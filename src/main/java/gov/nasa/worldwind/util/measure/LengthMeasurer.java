...
public class LengthMeasurer implements MeasurableLength {
    ...
    protected static List<? extends Position> subdividePositions(Globe globe, List<? extends Position> positions,
        ...
        if (followTerrain) {
            newPositions.add(new Position(pos1, globe.elevation(pos1.getLat(), pos1.getLon())));
        } else {
        ...
        for (int i = 1; i < count; i++) {
            ...
            if (arcLength > maxLength) {
                ...
                for (int j = 1; j < steps; j++) {
                    ...
                    double elevation;
                    if (followTerrain) {
                        elevation = globe.elevation(destLatLon.getLat(), destLatLon.getLon());
                    } else {
            ...
            if (followTerrain) {
                newPositions.add(new Position(pos2, globe.elevation(pos2.getLat(), pos2.getLon())));
            } else {
