 ...
 * </code>
 * </pre>
 * Callers can create a new RestorableSupport with no state content, or create a RestorableSupport from an existing XML
 * document string. Callers can then add state by name and value, and query state by name. RestorableSupport provides
 * convenience methods for addding and querying state values as Strings, Integers, Doubles, and Booleans.
 *
 * @author dcollins
    ...
    public static RestorableSupport parse(String stateInXml) {
        ...
        try {
            ...
            Document doc = docBuilder.parse(
                new InputSource(new StringReader(stateInXml)));
            return new RestorableSupport(doc);
        }
...
//        }
    ...
    public static void adjustTitleAndDisplayName(KV params) {
        String displayName = params.getStringValue(Keys.DISPLAY_NAME);
        if (displayName == null && params.get(Keys.TITLE) != null)
            params.set(Keys.DISPLAY_NAME, params.get(Keys.TITLE));
    }
...
//        }
        ...
        try {
            ...
            return (result instanceof Element) ? new StateObject((Element) result) : null;
        }
...
//        }
    ...
    protected boolean containsElement(Node elem) {
        if (elem == null) {
    ...
    public List<LatLon> getStateObjectAsLatLonList(StateObject stateObject) {
        ...
        List<LatLon> outList = new ArrayList<>(llsos.length);
    ...
    public OffsetsList getStateObjectAsOffsets(StateObject stateObject) {
        ...
        if (upperLeftOffset == null || upperRightOffset == null || lowerLeftOffset == null
            || lowerRightOffset == null) {
            String message = Logging.getMessage("generic.ConversionError", stateObject.getName());
            Logging.logger().log(java.util.logging.Level.SEVERE, message);
            return null;
        }
    ...
    public void addStateValueAsOffsetsList(StateObject context, String name, Map<Integer, OffsetsList> offsets) {
        ...
        RestorableSupport.StateObject stateObject = addStateObject(context, name);
        for (OffsetsList offsetsList : offsets.values()) {
            addStateValueAsOffsets(stateObject, "face", offsetsList);
        }
    }
     ...
     * RestorableSupport}. The <code>name</code> and simple String <code>value</code> of a
