...
public class WWUtil {
    ...
    public static void alignComponent(Component parent, Component child, String alignment) {
        ...
        Point parentLocation = parent != null ? parent.getLocation() : new Point(0, 0);
     ...
     * The <code>forceOverwrite</code> controls what to do if the destination list already contains values for
     * specified
    ...
    public static String stripLeadingPeriod(String s) {
        if (null != s && !s.isEmpty() && s.charAt(0) == '.')
            return s.substring(Math.min(1, s.length()));
        return s;
    }
    ...
    public static Long parseTimeString(String timeString) {
        ...
        if (isKMLTimeShift(timeString)) {
            ...
            timeString = newTimeString + timeString.substring(colonPosition + 1);
        }
    ...
    public static int sizeEstimate(Iterable x) {
