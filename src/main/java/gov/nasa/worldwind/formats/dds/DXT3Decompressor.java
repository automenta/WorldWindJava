...
public class DXT3Decompressor implements DXTDecompressor {
    ...
    protected static BufferedImage decodeDxt3Buffer(ByteBuffer buffer, int width, int height)
        ...
        if (width < DXT3Decompressor.DXT3_BLOCK_SIZE || height < DXT3Decompressor.DXT3_BLOCK_SIZE) {
        ...
        try {
            ...
            int numTilesWide = width / DXT3Decompressor.DXT3_BLOCK_SIZE;
            int numTilesHigh = height / DXT3Decompressor.DXT3_BLOCK_SIZE;

            // 8 bit per color ARGB packed in to an integer as a8r8g8b8
            int[] pixels = new int[DXT3Decompressor.DXT3_BLOCK_SIZE * width];
            ...
            for (int row = 0; row < numTilesHigh; row++) {
                for (int col = 0; col < numTilesWide; col++) {
                    ...
                    for (int k = DXT3Decompressor.DXT3_BLOCK_SIZE * DXT3Decompressor.DXT3_BLOCK_SIZE - 1; k >= 0; k--) {
...
//                      Color24 color = Color24.multiplyAlpha(lookupTable[colorIndex], alpha );
                        ...
                        int h = k / DXT3Decompressor.DXT3_BLOCK_SIZE, w = k % DXT3Decompressor.DXT3_BLOCK_SIZE;
                        int pixelIndex = h * width + (col * DXT3Decompressor.DXT3_BLOCK_SIZE + w);

                        pixels[pixelIndex] = pixel8888;
                    }
                }

                result.setRGB(0, row * DXT3Decompressor.DXT3_BLOCK_SIZE, width, DXT3Decompressor.DXT3_BLOCK_SIZE, pixels, 0, width);
            }
            return result;
        }
        catch (Throwable t) {
