...
public class DXT1Decompressor implements DXTDecompressor {
    ...
    protected static BufferedImage decodeDxt1Buffer(ByteBuffer buffer, int width, int height)
        ...
        if (width < DXT1Decompressor.DXT1_BLOCK_SIZE || height < DXT1Decompressor.DXT1_BLOCK_SIZE) {
        ...
        try {
            ...
            int numTilesWide = width / DXT1Decompressor.DXT1_BLOCK_SIZE;
            int numTilesHigh = height / DXT1Decompressor.DXT1_BLOCK_SIZE;

            // 8 bit per color RGB packed in to an integer as r8g8b8
            int[] pixels = new int[DXT1Decompressor.DXT1_BLOCK_SIZE * width];
            ...
            for (int row = 0; row < numTilesHigh; row++) {
                for (int col = 0; col < numTilesWide; col++) {
                    ...
                    for (int k = DXT1Decompressor.DXT1_BLOCK_SIZE * DXT1Decompressor.DXT1_BLOCK_SIZE - 1; k >= 0; k--) {
                        int h = k / DXT1Decompressor.DXT1_BLOCK_SIZE, w = k % DXT1Decompressor.DXT1_BLOCK_SIZE;
                        int pixelIndex = h * width + (col * DXT1Decompressor.DXT1_BLOCK_SIZE + w);
                ...
                result.setRGB(0, row * DXT1Decompressor.DXT1_BLOCK_SIZE, width, DXT1Decompressor.DXT1_BLOCK_SIZE, pixels, 0, width);
            }
            return result;
        }
        catch (Throwable t) {
    ...
    public BufferedImage decompress(ByteBuffer buffer, int width, int height) throws IllegalArgumentException {
        ...
        return DXT1Decompressor.decodeDxt1Buffer(buffer, width, height);
    }
}
