...
public class GeotiffImageReader extends ImageReader {
    ...
    public BufferedImage read(int imageIndex, ImageReadParam param) throws IOException {
        ...
        for (TiffIFDEntry entry : ifd) {
            switch (entry.tag) {
                case Tiff.Tag.IMAGE_WIDTH -> widthEntry = entry;
                case Tiff.Tag.IMAGE_LENGTH -> lengthEntry = entry;
                case Tiff.Tag.BITS_PER_SAMPLE -> bitsPerSampleEntry = entry;
                case Tiff.Tag.SAMPLES_PER_PIXEL -> samplesPerPixelEntry = entry;
                case Tiff.Tag.PHOTO_INTERPRETATION -> photoInterpEntry = entry;
                case Tiff.Tag.STRIP_OFFSETS -> stripOffsetsEntry = entry;
                case Tiff.Tag.STRIP_BYTE_COUNTS -> stripCountsEntry = entry;
                case Tiff.Tag.ROWS_PER_STRIP -> rowsPerStripEntry = entry;
                case Tiff.Tag.PLANAR_CONFIGURATION -> planarConfigEntry = entry;
                case Tiff.Tag.COLORMAP -> colorMapEntry = entry;
                case Tiff.Tag.SAMPLE_FORMAT -> sampleFormatEntry = entry;
            }
        }
