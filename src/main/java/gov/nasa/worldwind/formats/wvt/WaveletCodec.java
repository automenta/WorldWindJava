...
public class WaveletCodec {
    ...
    public static ByteBuffer save(WaveletCodec codec) {
        if (codec == null) {
    ...
    public static WaveletCodec loadPartial(ByteBuffer buffer, int resolution) {
        if (buffer == null) {
            String message = Logging.getMessage("nullValue.ByteBufferIsNull");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
        ...
        while (true) {
            ...
            for (int j = 0; j < xformYres; j++) {
                ...
                for (int i = 0; i < halfXformXres; i++) {
                    ...
                    // horizontally...
                    for (int k = 0; k < numBands; k++) {
                        float average = (imageData[k][indx1] + imageData[k][indx2]) / 2.0f;
            ...
            for (int subBlock = 0; subBlock < 3; subBlock++) {
                ...
                for (int j = 0; j < scale; j++) {
                    for (int i = 0; i < scale; i++, next++) {
                        ...
                        for (int k = 0; k < numBands; k++) {
                            codec.xform[k][next] = (byte) Math.max(Byte.MIN_VALUE,
                                Math.min(Byte.MAX_VALUE, Math.round(imageData[k][indx])));
                        }
                    }
                }
            }
    ...
    public BufferedImage reconstruct(int resolution) throws IllegalArgumentException {
        ...
        for (int j = 0; j < resolution; j++) {
            for (int i = 0; i < resolution; i++) {
                for (int k = 0; k < numBands; k++) {
                    imageBytes[k][j * resolution + i] = (byte) Math.max(0,
                        Math.min(255, imageData[k][j * resolution + i]));
                }
            }
        }
