...
/**
 * Implementation of {@link TacticalSymbol} that provides support for tactical symbols from the <a
 * href="https://en.wikipedia.org/wiki/NATO_Joint_Military_Symbology#MIL-STD-2525A">MIL-STD-2525</a> symbology set. See
 * the <a href="https://worldwind.arc.nasa.gov/java/tutorials/tactical-graphics/" target="_blank">Tutorial</a> for
 * instructions on using TacticalSymbol in an application.
 *
 * @author dcollins
...
public class MilStd2525TacticalSymbol extends AbstractTacticalSymbol {
    ...
    protected static final Collection<String> exerciseSymbols = new HashSet<>();

    static {
        // The MIL-STD-2525 symbols representing an echelon.
        MilStd2525TacticalSymbol.symbolEchelonMap.put("e-o-bj---------", SymbologyConstants.ECHELON_TEAM_CREW);

        // The MIL-STD-2525 symbols representing a exercise object.
        MilStd2525TacticalSymbol.exerciseSymbols.add("s-u-wmgx-------");
        MilStd2525TacticalSymbol.exerciseSymbols.add("s-u-wmmx-------");
        MilStd2525TacticalSymbol.exerciseSymbols.add("s-u-wmfx-------");
        MilStd2525TacticalSymbol.exerciseSymbols.add("s-u-wmx--------");
        MilStd2525TacticalSymbol.exerciseSymbols.add("s-u-wmsx-------");
    }
    ...
    protected void applyImplicitModifiers(AVList modifiers) {
        ...
        if (!modifiers.hasKey(SymbologyConstants.ECHELON)) {
            Object o = MilStd2525TacticalSymbol.symbolEchelonMap.get(maskedCode);
            if (o != null)
        ...
        if (!modifiers.hasKey(SymbologyConstants.FRAME_SHAPE)) {
            if (MilStd2525TacticalSymbol.exerciseSymbols.contains(maskedCode)) {
    ...
    protected void layoutGraphicModifiers(DrawContext dc, AVList modifiers, OrderedSymbol osym) {
        ...
        if (modifierCode != null) {
            this.addGlyph(dc, Offset.TOP_CENTER, Offset.BOTTOM_CENTER, modifierCode, null,
                AbstractTacticalSymbol.LAYOUT_RELATIVE, osym);
        }
        ...
        if (modifierCode != null) {
            this.addGlyph(dc, Offset.BOTTOM_CENTER, Offset.TOP_CENTER, modifierCode, null,
                AbstractTacticalSymbol.LAYOUT_RELATIVE, osym);
        }
    ...
    protected void layoutDynamicModifiers(DrawContext dc, AVList modifiers, OrderedSymbol osym) {
        ...
        if (o instanceof Angle) {
            ...
            if (this.useGroundHeadingIndicator) {
                ...
                this.addLine(dc, Offset.BOTTOM_CENTER, points, AbstractTacticalSymbol.LAYOUT_RELATIVE, points.size() - 1, osym);
            } else {
    ...
    protected void layoutTextModifiers(DrawContext dc, AVList modifiers, OrderedSymbol osym) {
        ...
        if (frameShapeFont == null)
            frameShapeFont = MilStd2525TacticalSymbol.DEFAULT_FRAME_SHAPE_FONT;

        // Quantity modifier layout. Placed at the top of the symbol layout.
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.QUANTITY, 9);
        if (!sb.isEmpty()) {
            this.addLabel(dc, Offset.TOP_CENTER, Offset.BOTTOM_CENTER, sb.toString(), font, null,
                AbstractTacticalSymbol.LAYOUT_RELATIVE,
                osym);
            sb.delete(0, sb.length());
        }

        // Special C2 Headquarters modifier layout. Centered on the icon.
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.SPECIAL_C2_HEADQUARTERS, 9);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.FRAME_SHAPE, null);
        String s = MilStd2525TacticalSymbol.getReinforcedReducedModifier(modifiers,
            SymbologyConstants.REINFORCED_REDUCED);
        if (s != null)
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.STAFF_COMMENTS, 20);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.ADDITIONAL_INFORMATION, 20);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.HIGHER_FORMATION, 21);
        if (!sb.isEmpty()) {
        ...
        // layout.
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.EVALUATION_RATING, 2);
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.COMBAT_EFFECTIVENESS, 3);
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.SIGNATURE_EQUIPMENT, 1);
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.HOSTILE_ENEMY, 3);
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.IFF_SIF, 5);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.DATE_TIME_GROUP, 16);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.ALTITUDE_DEPTH, 14);
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.LOCATION, 19);
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.TYPE, 24);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.UNIQUE_DESIGNATION, 21);
        if (!sb.isEmpty()) {
        ...
        MilStd2525TacticalSymbol.appendTextModifier(sb, modifiers, SymbologyConstants.SPEED, 8);
        if (!sb.isEmpty()) {
