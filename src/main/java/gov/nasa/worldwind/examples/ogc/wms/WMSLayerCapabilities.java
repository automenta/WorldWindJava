...
package gov.nasa.worldwind.examples.ogc.wms;

import gov.nasa.worldwind.examples.ogc.OGCBoundingBox;
...
public class WMSLayerCapabilities extends AbstractXMLEventParser {
    ...
    @SuppressWarnings("UnusedDeclaration")
    protected void parseScaleHint(XMLEventParserContext ctx, XMLEvent bboxEvent) {
        Iterator iter = bboxEvent.asStartElement().getAttributes();
        if (iter == null)
            return;
    ...
    protected void addDimensions(Collection<WMSLayerDimension> dims) {
        if (dims.isEmpty())
            return;
    ...
    protected void addExtents(Collection<WMSLayerExtent> inExtents) {
        if (inExtents.isEmpty())
            return;
    ...
    protected void addAttributions(Iterable<WMSLayerAttribution> attribs) {
    ...
    protected void addAuthorityURLs(Iterable<WMSAuthorityURL> urls) {
    ...
    protected void addIdentifiers(Iterable<WMSLayerIdentifier> ids) {
    ...
    protected void addMetadataURLs(Iterable<WMSLayerInfoURL> urls) {
    ...
    protected void addFeatureListURLs(Iterable<WMSLayerInfoURL> urls) {
        for (WMSLayerInfoURL url : urls) {
    ...
    protected void addDataURLs(Iterable<WMSLayerInfoURL> urls) {
        for (WMSLayerInfoURL url : urls) {
            this.addDataURL(url);
        }
    }
    ...
    protected void addLayers(Iterable<WMSLayerCapabilities> inLayers) {
    ...
    protected void addStyles(Iterable<WMSLayerStyle> inStyles) {
    ...
    protected void addBoundingBoxes(Iterable<OGCBoundingBox> boxes) {
    ...
    protected void addSRS(Iterable<String> srss) {
        for (String c : srss) {
            this.addSRS(c);
        }
    }
    ...
    protected void addCRS(Iterable<String> crss) {
        for (String c : crss) {
            this.addCRS(c);
        }
    }
