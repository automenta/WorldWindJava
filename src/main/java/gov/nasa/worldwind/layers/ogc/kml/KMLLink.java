...
public class KMLLink extends KMLAbstractObject {
    ...
    protected Long computeRefreshTime() {
        ...
        if (KMLConstants.ON_INTERVAL.equals(this.getRefreshMode())) {
            ...
            refreshTime = ri != null ? this.updateTime.get() + (long) (ri * 1000.0d) : null;
        }
     ...
     * <code>href</code>, the <code>viewFormat</code> and the <code>httpQuery</code> for form an absolute URL string.
     * If the the <code>href</code> contains a query string, the <code>viewFormat</code> and <code>httpQuery</code> are
     ...
     * the specified <code>DrawContext</code>'s current viewing parameters, and substitutes parameters in
     * <code>httpQuery</code> according to the current <code>{@link Configuration}</code>
     * parameters.
     *
     * @param dc the <code>DrawContext</code> used to determine the current view parameters.
        ...
        if (!WWUtil.isEmpty(viewRefreshMode) && !KMLConstants.NEVER.equals(viewRefreshMode)) {
            ...
            if (!WWUtil.isEmpty(s)) {
                ...
                if (!queryString.isEmpty() && queryString.charAt(queryString.length() - 1) != '&')
                    queryString.append('&');
                queryString.append(s, !s.isEmpty() && s.charAt(0) == '&' ? 1 : 0, s.length());
            }
        }
