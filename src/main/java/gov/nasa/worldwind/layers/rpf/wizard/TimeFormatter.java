...
public class TimeFormatter {
    ...
    private static long[] millisToHMS(long millis) {
        return new long[] {
            (long) (Math.floor(millis / (double) ONE_HOUR) % 60.0d),  // hours
            (long) (Math.floor(millis / (double) ONE_MINUTE) % 60.0d),  // minutes
    ...
    public static String formatEstimate(long millis) {
        ...
        // Report time in ten-minute increments.
            ...
            millis = 10L * ONE_MINUTE * Math.round(millis / (10.0d * ONE_MINUTE));
            long m = millis / ONE_MINUTE;
            result = "about " + m + " minutes";
        }
        // Report time in half-hour increments.
        else {
            millis = 30L * ONE_MINUTE * Math.round(millis / (30.0d * ONE_MINUTE));
