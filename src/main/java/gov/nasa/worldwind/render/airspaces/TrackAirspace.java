...
public class TrackAirspace extends AbstractAirspace {
    protected List<gov.nasa.worldwind.render.airspaces.Box> legs = new ArrayList<>();
    ...
    public TrackAirspace(TrackAirspace source) {
        ...
        for (gov.nasa.worldwind.render.airspaces.Box leg : source.legs) {
            this.legs.add(new gov.nasa.worldwind.render.airspaces.Box(leg));
        }
    ...
    public void setLegs(Collection<gov.nasa.worldwind.render.airspaces.Box> legs) {
        this.legs.clear();
        this.addLegs(legs);
    }

    protected void addLegs(Iterable<gov.nasa.worldwind.render.airspaces.Box> newLegs) {
        if (newLegs != null) {
            for (gov.nasa.worldwind.render.airspaces.Box b : newLegs) {
                if (b != null)
                    this.addLeg(b);
            }
        }
    ...
    public gov.nasa.worldwind.render.airspaces.Box addLeg(LatLon start, LatLon end, double lowerAltitude,
        ...
        gov.nasa.worldwind.render.airspaces.Box leg = new gov.nasa.worldwind.render.airspaces.Box();
    ...
    protected void addLeg(gov.nasa.worldwind.render.airspaces.Box leg) {
        if (leg == null) {
    ...
    public void setEnableCenterLine(boolean enable) {
        ...
        for (gov.nasa.worldwind.render.airspaces.Box leg : this.legs) {
     ...
     * @see #setSmallAngleThreshold(Angle)
     */
    ...
    public void setAltitudes(double lowerAltitude, double upperAltitude) {
        ...
        for (gov.nasa.worldwind.render.airspaces.Box l : this.legs) {
    ...
    public void setAlwaysOnTop(boolean alwaysOnTop) {
        ...
        for (gov.nasa.worldwind.render.airspaces.Box l : this.getLegs()) {
    ...
    public boolean isAirspaceVisible(DrawContext dc) {
        ...
        for (gov.nasa.worldwind.render.airspaces.Box b : this.legs) {
    ...
    public Position getReferencePosition() {
        List<LatLon> locations = new ArrayList<>(2 * this.legs.size());
        for (gov.nasa.worldwind.render.airspaces.Box box : this.legs) {
    ...
    protected Extent computeExtent(Globe globe, double verticalExaggeration) {
        List<gov.nasa.worldwind.render.airspaces.Box> trackLegs = this.getLegs();
    ...
    protected void doUpdateLegs() {
        ...
        for (int i = 0; i < this.legs.size() - 1; i++) {
            gov.nasa.worldwind.render.airspaces.Box leg = this.legs.get(i);
            gov.nasa.worldwind.render.airspaces.Box nextLeg = this.legs.get(i + 1);
     ...
     * of
     * <code>leg1</code> and the beginning of <code>leg2</code>. Without joining the adjacent edges, the geometry of
     * the two adjacent boxes contains a gap on one side and an intersection on the other.
     * <p>
