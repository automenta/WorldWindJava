...
public class SurfaceText extends AbstractSurfaceObject implements GeographicText, Movable, Draggable {
     ...
     */
    public SurfaceText(CharSequence text, Position position) {
        this.setText(text);
     ...
     */
    public SurfaceText(CharSequence text, Position position, Font font, Color color) {
        this.setText(text);
        this.setPosition(position);
        this.setFont(font);
        this.setColor(color);
    ...
    protected void applyDrawTransform(DrawContext dc, SurfaceTileDrawContext sdc) {
        ...
        if (this.spansAntimeridian &&
            Math.signum(sdc.getSector().lonMin().degrees) != Math.signum(this.drawLocation.longitude.degrees)) {
