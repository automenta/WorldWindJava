 ...
 * its own methods though. </p>  <p><b>Usage:</b></p>  <p>Instantiation:</p> <p> The MLTR needs a Font or a TextRenderer
 * to be instantiated. This will be the font used for text drawing, wrapping and measuring. For HTML methods this font
 * will be considered as the document default font. </p>
 * <pre>
 ...
 * </pre>
 * <p> The textBounds rectangle returned contains the width and height of the text as it would be drawn with the
 * current font. </p> <p> Note that textBounds.minX is the number of lines found and textBounds.minY is the maximum line
 * height for the font used. This value can be safely used as the lineHeight argument when drawing - or can even be
 * ommited after a getBounds: draw(text, x, y); ... </p>  <p><b>HTML support</b></p> <p> Supported tags are: <ul>
...
public class MultiLineTextRenderer {
     ...
     */
    public static boolean containsHTML(CharSequence text) {
        if (text == null) {
        ...
        Matcher matcher = MultiLineTextRenderer.SGMLPattern.matcher(text);
        return matcher.find();
    }
     ...
     */
    public static String getAttributeFromTagHTML(CharSequence text, String attributeName) {
    ...
    protected static Rectangle2D getWordBoundsHTML(String word, DrawState ds) {
        ...
        Matcher matcher = MultiLineTextRenderer.SGMLOrSpacePattern.matcher(word);  // html tags or spaces
        while (matcher.find()) {
            if (!matcher.group().equals(" "))  // html tag - not a space
            {
                ...
                height = Math.max(MultiLineTextRenderer.getMaxLineHeight(ds.textRenderer), height);
                start = matcher.end();
            }
        }
    ...
    protected static void pickWord(String word, String hyperlink, double drawX, double drawY, Rectangle2D wordBounds,
        ...
        MultiLineTextRenderer.drawFilledRectangle(dc, drawX, drawY - wordBounds.getHeight() / 4,  // TODO: handle font descent properly
    ...
    public Rectangle getBounds(String text) {
        ...
        maxLineHeight = (int) Math.max(MultiLineTextRenderer.getMaxLineHeight(this.textRenderer), maxLineHeight);
    ...
    protected void drawLineWithUniqueColors(String text, int x, int y,
        ...
        while (start < source.length()) {
            ...
            // Draw word rectangle
            gl.glColor3ub((byte) color.getRed(), (byte) color.getGreen(), (byte) color.getBlue());
            MultiLineTextRenderer.drawFilledRectangle(dc, drawX + wordBounds.getX(), y - wordBounds.getHeight() - wordBounds.getY(),
                wordBounds.getWidth(), wordBounds.getHeight());
    ...
    public String wrap(String text, int width, int height) {
        ...
        double maxLineHeight = MultiLineTextRenderer.getMaxLineHeight(this.textRenderer);
    ...
    protected Rectangle2D getLineBoundsHTML(String line, DrawState ds) {
        ...
        while (wi.hasNext()) {
            ...
            Rectangle2D wordBounds = MultiLineTextRenderer.getWordBoundsHTML((String) wi.next(), ds);
    ...
    protected String wrapTextHTML(String text, double width, double height, DrawState ds) {
        ...
        String[] lines = text.split("\n");
        for (String line : lines) {
            wrappedText.append(wrappedText.isEmpty() ? "" : "\n");
    ...
    protected String wrapLineHTML(String line, double width, DrawState ds) {
        ...
        while (wi.hasNext()) {
            ...
            wordWidth = MultiLineTextRenderer.getWordBoundsHTML(word, savedState).getWidth();
    ...
    protected void drawLineHTML(String line, double x, double y, DrawState ds) {
        ...
        while (wi.hasNext()) {
            word = wi.next();
            wordBounds = MultiLineTextRenderer.getWordBoundsHTML(word, new DrawState(ds));
            if (this.isPicking)
    ...
    protected void pickWordPartHTML(String word, double x, double y, Rectangle2D partBounds, DrawState ds,
        ...
        MultiLineTextRenderer.pickWord(word, hyperlink, x, y, pickBounds,
    ...
    public static class WordIteratorHTML implements Iterator<String> {
        ...
        public WordIteratorHTML(String text) {
            Matcher matcher = WordIteratorHTML.SGMLOrSpacePattern.matcher(text);
    ...
    protected class DrawState {
        ...
        public void updateFromHTMLText(CharSequence text, boolean startStopRendering) {
