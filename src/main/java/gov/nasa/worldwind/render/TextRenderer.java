 ...
 * <PRE>
 * renderer.beginRendering(drawable.getWidth(), drawable.getHeight()); // optionally set the color
 * renderer.setColor(1.0f, 0.2f, 0.2f, 0.8f); renderer.draw("Text to draw", xPosition, yPosition); // ... more draw
 * commands, color changes, etc. renderer.endRendering();
 * </PRE>
 * <p>
 * Unless you are sharing textures and display lists between OpenGL contexts, you do not need to call the {@link
 * #dispose dispose} method of the TextRenderer; the OpenGL resources it uses internally will be cleaned up
 * automatically when the OpenGL context is destroyed. <P>
 *
 * <b>Note</b> that the TextRenderer may cause the vertex and texture
 * coordinate array buffer bindings to change, or to be unbound. This is important to note if you are using Vertex
 * Buffer Objects (VBOs) in your application. <P>
 * <p>
...
public class TextRenderer {
    ...
    private static final boolean DISABLE_GLYPH_CACHE;
     ...
     * RenderDelegate. Equivalent to <code>TextRenderer(font, false, false)</code>.
     *
     ...
     * smoothing when rendering the TextureRenderer's contents at a distance. Equivalent to <code>TextRenderer(font,
     * false, false)</code>.
     *
     * @param font   the font to render with
     ...
     * flags provide control over the same properties at the Java 2D level. No mipmap support is requested. Equivalent
     * to
     ...
     * <code>antialiased</code> and <code>useFractionalMetrics</code>
     * flags provide control over the same properties at the Java 2D level. The <code>renderDelegate</code> provides
     ...
     * <code>antialiased</code> and <code>useFractionalMetrics</code>
     * flags provide control over the same properties at the Java 2D level. The <code>renderDelegate</code> provides
     * more control over the text rendered. If <CODE>mipmap</CODE> is true, attempts to use OpenGL's automatic mipmap
     * generation for better smoothing when rendering the TextureRenderer's contents at a distance.
     *
     * @param font                 the font to render with
     * @param antialiased          whether to use antialiased fonts
     * @param useFractionalMetrics whether to use fractional font metrics at the Java 2D level
    ...
    /**
     * Returns the bounding rectangle of the given String, assuming it was rendered at the origin.See {@link
     * #getBounds(CharSequence) getBounds(CharSequence)}.
     *
     ...
     * considered transient and may become invalidated between {@link #beginRendering beginRendering} / {@link
     * #endRendering endRendering} pairs.
     *
     ...
     * #draw(CharSequence, int, int) draw(CharSequence, int, int)}.
     *
...
//    }
    ...
    private void clearUnusedEntries() {
        final Collection<Rect> deadRects = new ArrayList<>();
    ...
    private boolean is15Available(GLBase gl) {
     ...
     * them using the string-by-string method. <P>
     * <p>
