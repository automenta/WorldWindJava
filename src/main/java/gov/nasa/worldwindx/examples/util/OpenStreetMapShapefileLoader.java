...
public class OpenStreetMapShapefileLoader {
    ...
    /**
     * Creates a {@link Layer} from an OpenStreetMap Shapefile source of placemarks. The
     ...
     * from the Shapefile record attribute key "type" as follows: <table> <caption style="font-weight:
     * bold;">Mapping</caption>
    ...
    /**
     * Creates a {@link Layer} from an OpenStreetMap Shapefile of placemarks.
     * <p>
     * The returned Layer renders each Shapefile record as a surface circle with an associated screen label. The label
     * text is taken from the Shapefile record attribute key "name". This determines each surface circle's appearance
     * from the Shapefile record attribute key "type" as follows: <table> <caption style="font-weight:
     * bold;">Mapping</caption><tr><th>Type</th><th>Color</th></tr>
     * <tr><td>hamlet</td><td>Black</td></tr> <tr><td>village</td><td>Green</td></tr>
     * <tr><td>town</td><td>Cyan</td></tr> <tr><td>city</td><td>Yellow</td></tr> </table>
     *
    ...
    public static Layer makeLayerFromOSMPlacesShapefile(Shapefile shp) {
        ...
        OSMShapes[] shapeArray =
            {
                new OSMShapes(Color.BLACK, 0.3, 30.0e3), // hamlet
                new OSMShapes(Color.GREEN, 0.5, 100.0e3), // village
                new OSMShapes(Color.CYAN, 1, 500.0e3), // town
                new OSMShapes(Color.YELLOW, 2, 3000.0e3) // city
            };
        ...
        for (OSMShapes shapes : shapeArray) {
            ...
            BufferedImage image = PatternFactory.createPattern(PatternFactory.PATTERN_CIRCLE, 0.8f, shapes.foreground);
            SurfaceIcons sis = new SurfaceIcons(image, shp.getPointBuffer().getLocations());
            sis.setMaxSize(4.0e3 * shapes.scale); // 4km
    ...
    protected static class OSMShapes {
        public final List<LatLon> locations = new ArrayList<>();
