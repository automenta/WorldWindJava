...
public class ExampleUtil {
     ...
     * path may be one of the following: <ul> <li>{@link InputStream}</li> <li>{@link java.net.URL}</li>
     * <li>absolute {@link java.net.URI}</li> <li>{@link File}</li> <li>{@link String} containing a valid URL
    ...
    public static void goTo(WorldWindow wwd, Sector sector) {
        ...
        wwd.getView().goTo(new Position(sector.getCentroid(), 0.0d), zoom);
    }
    ...
    public static double[] createRandomGridValues(int width, int height, double min, double max, int numIterations,
        ...
        for (int i = 0; i < numIterations; i++) {
            double offset = 1.0d - (i / (double) numIterations);
